...
appstore: appstore-check
	...
	@echo "upload with:"
	@echo "  xcrun altool --upload-app -f $(APPSTORE_PKG) -t macos \"
	@echo "    -u <your-apple-id> -p \"@keychain:AC_PASSWORD\""
	@echo ""
	@echo "  altool needs its own credential — notarytool's --keychain-profile"
	@echo "  is not accepted. Store one once with:"
	@echo "    xcrun altool --store-password-in-keychain-item AC_PASSWORD \"
	@echo "      -u <your-apple-id> -p <app-specific-password>"
	@echo ""
	@echo "  Or use the Transporter app, which needs no terminal credentials."
